...
from transformers.models.llama.modeling_llama import LlamaPreTrainedModel, LlamaForCausalLM

from .opt import OPTDecoder, OPTForCausalLM, OPTModel
from .roberta import RobertaLMHead, RobertaModel, RobertaPreTrainedModel
...
def model_for_prompting_forward(
    ...
    if sfc_input_ids is not None:
        with torch.no_grad():
            logits = model_for_prompting_forward(model, input_ids=sfc_input_ids,
                                                 attention_mask=sfc_attention_mask,
                                                 mask_pos=sfc_mask_pos)[0]
...
class RobertaModelForPromptFinetuning(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        logger.warn("By default for RoBERTa models the input embeddings and the output embeddings "
                    "are NOT tied!!!!")
        self.num_labels = config.num_labels
        print(config)
        self.model_type = config.model_type
...
class OPTModelForPromptFinetuning(OPTForCausalLM):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, mask_pos=None,
                labels=None, sfc_input_ids=None, sfc_attention_mask=None, sfc_mask_pos=None):
        return model_for_prompting_forward(self, input_ids, attention_mask, token_type_ids,
                                           mask_pos, labels, sfc_input_ids=sfc_input_ids,
...
class LlamaModelForPromptFinetuning(LlamaPreTrainedModel):
    def __init__(self, config):
        ...
        self.base_model = LlamaForCausalLM.from_pretrained(config._name_or_path, config=config)
    ...
    def get_lm_head_fn(self):
        return lambda hidden: self.base_model.lm_head(hidden)

    def forward(self, input_ids=None, attention_mask=None,
                token_type_ids=None, mask_pos=None, labels=None,
                sfc_input_ids=None, sfc_attention_mask=None, sfc_mask_pos=None):
